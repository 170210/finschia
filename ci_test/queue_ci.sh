...
sum_msg=`jq -nc '{sum:{}}'`
fnsad query wasm contract-state smart $CONTRACT_ADDRESS $sum_msg

reducer_msg=`jq -nc '{reducer:{}}'`
fnsad query wasm contract-state smart $CONTRACT_ADDRESS $reducer_msg

list_msg=`jq -nc '{list:{}}'`
fnsad query wasm contract-state smart $CONTRACT_ADDRESS $list_msg
